                ...
                const uint8_t sk[KYBER_INDCPA_SECRETKEYBYTES])
{
  ...
  t1 = time_us_64();
  dec_prof.matmul += (t1 - t0);

  t0 = time_us_64();
  poly_invntt_tomont(&mp);
  t1 = time_us_64();
  dec_prof.invntt += (t1 - t0);

  t0 = time_us_64();
  poly_sub(&mp, &v, &mp);
  poly_reduce(&mp);
  t1 = time_us_64();
  dec_prof.sub_reduce += (t1 - t0);

  t0 = time_us_64();
  poly_tomsg(m, &mp);
  t1 = time_us_64();
  dec_prof.tomsg += (t1-t0);
