                                ...
                                unsigned int buflen)
{
  ...
  {
    val0 = ((buf[pos + 0] >> 0) | ((uint16_t)buf[pos + 1] << 8)) & 0xFFF; // Parallalization indcpa_1
...
void core1_worker()
{
  ...
  core1_data_t *data = (core1_data_t *)multicore_fifo_pop_blocking();
                           ...
                           const uint8_t coins[KYBER_SYMBYTES])
{
  ...
  static volatile core1_data_t core1_data;
  core1_data.buf = buf;
  core1_data.a = a;
...
// Setup data packet for core1
  ...
  polyvec_add(&b, &b, &ep); // Parallalization indcpa_7.1
  poly_add(&v, &v, &epp);   // Parallalization indcpa_7.2.1
  poly_add(&v, &v, &k);     // Parallalization indcpa_7.2.2
  polyvec_reduce(&b);       // Parallalization indcpa_8
  poly_reduce(&v);          // Parallalization indcpa_8
